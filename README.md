...
Both tracks use Python scripts (no notebooks) and rely on standard ML
frameworks (PyTorch, neuraloperator). No model weights or pre-computed
result files are bundled. All results are regenerated by running the
scripts from scratch with the seeds specified in each script's CONFIG
section.
